...
notify_qt_color_change() {
    ...
    # kdeglobals and CdlNiri.colors are already generated by matugen above.
    # Notify running KDE/Qt apps to reload colors via the KDE settings notification bus.
    dbus-send --session --type=signal /KGlobalSettings \
        org.kde.KGlobalSettings.notifyChange int32:0 int32:0 2>/dev/null || true
    # plasma-integration also watches kdeglobals via inotify — touch to trigger it.
    touch "$XDG_CONFIG_HOME/kdeglobals"
    touch "${XDG_DATA_HOME:-$HOME/.local/share}/color-schemes/CdlNiri.colors"
}
