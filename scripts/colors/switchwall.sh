...
check_and_prompt_upscale() {
    local img="$1"
    min_width_desired="$(niri msg -j outputs | jq '[.[] | .logical.width] | max' | xargs)" # max monitor width
    min_height_desired="$(niri msg -j outputs | jq '[.[] | .logical.height] | max' | xargs)" # max monitor height
...
for monitor in \$(niri msg -j outputs | jq -r 'keys[]'); do
...
switch() {
    ...
    # niri: get focused output geometry; no cursor pos IPC → use center of screen
    _focused_output=$(niri msg -j focused-output 2>/dev/null)
    scale=$(echo "$_focused_output" | jq -r '.logical.scale // 1')
    screenx=$(echo "$_focused_output" | jq -r '.logical.x // 0')
    screeny=$(echo "$_focused_output" | jq -r '.logical.y // 0')
    screensizex=$(echo "$_focused_output" | jq -r '.logical.width // 1920')
    screensizey=$(echo "$_focused_output" | jq -r '.logical.height // 1080')
    # niri has no cursor position IPC — default to screen center
    cursorposx=$(( screensizex / 2 ))
    cursorposy=$(( screensizey / 2 ))
    ...
    max_width_desired="$(niri msg -j outputs | jq '[.[] | .logical.width] | min')"
    max_height_desired="$(niri msg -j outputs | jq '[.[] | .logical.height] | min')"
